...
library(tidyverse); library(adaptBayes)
...
y_hist = historical$y_hist;
y_curr = current$y_curr;
x_hist_orig = as.matrix(historical[,2:5]);
x_curr_orig = as.matrix(current[,2:5]);
colnames(x_hist_orig) = 
  colnames(x_curr_orig) = orig_covariates;
x_curr_aug = as.matrix(current[,6:11]);
